...
class FRModel(Model):
    def __init__(self,
        ...
        #self._report_cnn = CnnEncoder(representation_dim, 100)
        self._comment_cnn = CnnEncoder(representation_dim, 100)
        ...
        self._projector = FeedForward(self._seq2vec.get_output_dim(), 1,
                                      50,
                                      Activation.by_name("relu")(), dropout)

        self._predictor = nn.Linear(50, self._num_class)
        ...
        self._loss = torch.nn.CrossEntropyLoss()#weight=torch.tensor([0.5,1])
    ...
    def forward(self,
        ...
        if self._golden_instances is not None:
            ...
            project_out = self._projector(rnn_vec1)
